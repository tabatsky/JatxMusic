...
public class ReceiverController extends Thread {
	...
	volatile WeakReference<ServiceController> ref;
	
	volatile boolean finishFlag;
	
	String host;
	
	public ReceiverController(String hostname, ServiceController serviceController) {
		host = hostname;
		finishFlag = false;
	...
	public void setVolume(final int vol) {
		...
		final ServiceController serviceController = ref.get();
		if (serviceController !=null) {
			serviceController.setVolume(vol);
		}
	}
